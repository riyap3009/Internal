...
def save():
    ...
    with open("workout.txt", "a") as file:
        file.write(f"{current_user}:{x.date()}:{part}:{workout}:{value}\n")
...
Button(
    ...
    relief="groove",
    command=login
).grid(row=3, column=1, columnspan=3, pady=10)
...
row_num = 2# starts at row 2 because row 1 is taken
for part in parts:
    Radiobutton(
        workout_frame,
    ...
    row_num += 2 #start at row 2 and continues however many rows in the list parts
